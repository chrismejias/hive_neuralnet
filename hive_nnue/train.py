"""
CLI entry point for NNUE-based Hive training.

Usage:
    python -m hive_nnue --iterations 20 --games 50 --simulations 100
    python -m hive_nnue --preset large
    python -m hive_nnue --hidden-dims 1024 512 256
    python -m hive_nnue --resume checkpoints_nnue/hive_nnue_checkpoint_0010.pt
    python -m hive_nnue --device cuda
"""
...
from hive_nnue.nnue_net import NNUEConfig
from hive_nnue.nnue_trainer import NNUETrainConfig, NNUETrainer


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train an NNUE-based Hive AI via AlphaZero self-play.",
    ...
    # NNUE architecture
    parser.add_argument(
        "--preset", choices=["small", "large"], default=None,
        help="Network size preset. Overrides --hidden-dims.",
    )
    parser.add_argument(
        "--hidden-dims", type=int, nargs="+", default=None,
        help="Hidden layer dimensions (e.g. --hidden-dims 512 256).",
    )
    parser.add_argument(
        "--dropout", type=float, default=0.1,
        help="Dropout rate in backbone layers.",
    )
    ...
    # Checkpointing
    parser.add_argument(
        "--checkpoint-dir", type=str, default="checkpoints_nnue",
...
def _build_net_config(args: argparse.Namespace) -> NNUEConfig:
    """Build NNUEConfig from CLI arguments."""
    if args.preset == "large":
        config = NNUEConfig.large()
    elif args.preset == "small":
        config = NNUEConfig.small()
    elif args.hidden_dims is not None:
        config = NNUEConfig(hidden_dims=args.hidden_dims)
    else:
        config = NNUEConfig.small()
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.resume:
        ...
        trainer = NNUETrainer.from_checkpoint(
            args.resume, config_overrides=overrides or None
        )
        print(f"Resuming from checkpoint: {args.resume}")
    else:
        train_config = NNUETrainConfig(
        ...
        trainer = NNUETrainer(config=train_config, net_config=net_config)

    # Print config summary
    device = trainer.device
    print(f"\n{'='*60}")
    print("Hive NNUE Training Configuration")
    print(f"{'='*60}")
    print(f"  Device:       {device} ({device_summary(device)})")
    print(f"  AMP:          {trainer.use_amp}")
    print(f"  Hidden dims:  {trainer.net_config.hidden_dims}")
    print(f"  Dropout:      {trainer.net_config.dropout}")
