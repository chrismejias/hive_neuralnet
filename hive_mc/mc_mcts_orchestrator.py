...
class MCMCTSOrchestrator:
    """Gumbel AlphaZero with true MCTS tree search, MC (move-conditioned)."""

    def __init__(
        self,
        net: HiveMoveTransformer,
        config: MCMCTSConfig | None = None,
    ) -> None:
        self.ext        = hive_gpu.load_extension()
        self.config     = config or MCMCTSConfig()
        self.net        = net
    ...
    def self_play_batch(
        ...
        while bool(active_mask.any().item()):
            ...
            # Games without legal moves get finalised this iteration
            ...
            # ── Screening-head priors (per legal move) ────────────────
            move_feats = parse_move_features(legal_moves, num_legal, dev)
            with torch.no_grad():
                _, root_cls = self.net.encode(root_batch)
                screen_flat = self.net.screen(
                    root_cls, root_batch.global_features, move_feats,
                )   # (total_actions,) logits

            # flat → padded (B, max_n) logits; softmax across legal slots
            screen_pad = flat_to_padded(
                screen_flat, num_legal, pad_value=float("-inf"),
            )   # (B, max_n)

            # Pad / trim to MAX_LEGAL for dense-priors kernel
            priors_per_legal = torch.zeros(
                B, self._max_legal, dtype=torch.float32, device=dev,
            )
            legal_logits = torch.full(
                (B, self._max_legal), -1e30, dtype=torch.float32, device=dev,
            )
            eff_n = min(max_n, self._max_legal)
            if eff_n > 0:
                legal_logits[:, :eff_n] = screen_pad[:, :eff_n]
                ...
                legal_logits = torch.where(
                    valid_slot, legal_logits,
                    torch.full_like(legal_logits, -1e30),
                )
                priors_per_legal = torch.softmax(legal_logits, dim=-1)
                priors_per_legal = torch.where(
                    valid_slot, priors_per_legal, torch.zeros_like(priors_per_legal),
                )

            # ── Gumbel top-k over legal slots ─────────────────────────
            u = torch.rand(B, self._max_legal, device=dev).clamp(1e-4, 1 - 1e-4)
            gumbel = -torch.log(-torch.log(u))
            slot_idx = torch.arange(self._max_legal, device=dev).unsqueeze(0)
            valid_slot = slot_idx < n_per_game.unsqueeze(1)
            gumbel = torch.where(valid_slot, gumbel, torch.full_like(gumbel, -1e30))
            perturbed = gumbel + legal_logits       # (B, MAX_L)
            _, topk_slots = torch.topk(perturbed, max_k, dim=1)     # (B, max_k)
            ...
            alive_mask = torch.zeros(B, self._max_legal, dtype=torch.int8, device=dev)
            ...
            chosen_slot = torch.argmax(final_sigma, dim=1)   # (B,)
            ...
            # Fallback uniform for games that somehow have zero visits
            # (shouldn't happen in normal operation, but guard anyway).
            ...
            # ── Record history (state bytes + per-legal policy) ──────
            ...
            # For every active game, use chosen_slot to select from legal_moves
            ...
            # Compute chosen child nodes for re-rooting
        ...
        for i in range(B):
            ...
            for state_bytes, target_pi in histories[i]:
                ...
                examples[i].append(MCTrainingExample(
    ...
    def _run_simulations(
        ...
        for wave in range(num_waves):
            ...
            # Encode leaves + screening priors
            leaf_batch = self.encoder.encode_batch(leaf_states[:total], total)
            move_feats = parse_move_features(legal_moves, num_legal, "cuda")
            with torch.no_grad():
                _, leaf_cls = self.net.encode(leaf_batch)
                leaf_vals = self.net.value_head(
                    leaf_cls, leaf_batch.global_features,
                ).squeeze(-1).float()
                screen_flat = self.net.screen(
                    leaf_cls, leaf_batch.global_features, move_feats,
                )
            ...
            # Build per-legal priors for leaves
            eff_n = min(screen_pad.shape[1], self._max_legal)
            priors_leaf = torch.zeros(
                total, self._max_legal, dtype=torch.float32, device="cuda",
            )
            leaf_logits = torch.full(
                (total, self._max_legal), -1e30, dtype=torch.float32, device="cuda",
            )
            if eff_n > 0:
                leaf_logits[:, :eff_n] = screen_pad[:, :eff_n]
                n_leaf = num_legal.to(torch.int64)
                slot_idx = torch.arange(self._max_legal, device="cuda").unsqueeze(0)
                valid_slot = slot_idx < n_leaf.unsqueeze(1)
                leaf_logits = torch.where(
                    valid_slot, leaf_logits,
                    torch.full_like(leaf_logits, -1e30),
                )
                priors_leaf = torch.softmax(leaf_logits, dim=-1)
                priors_leaf = torch.where(
