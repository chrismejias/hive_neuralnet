"""
NNUE-based AlphaZero trainer for Hive.

Follows the same self-play -> train -> arena -> promote cycle as
the CNN and GNN trainers, but uses flat NNUE feature vectors instead
of grid tensors or graphs.

Reuses from hive_engine:
    - compute_loss() (same policy/value loss)
    - MCTS (duck-typed with NNUEEncoder/HiveNNUE)
...
from hive_nnue.nnue_replay_buffer import (
    NNUETrainingExample,
    NNUEReplayBuffer,
)
...
@dataclass
class NNUETrainConfig:
    """Configuration for NNUE training pipeline."""
    ...
    # Checkpointing
    checkpoint_dir: str = "checkpoints_nnue"
...
class NNUETrainer:
    """
    AlphaZero-style self-play trainer using NNUE feature vectors.

    Same pipeline as the other trainers:
        1. Self-play -> collect (features, policy, value) examples
    ...
    def __init__(
        self,
        config: NNUETrainConfig | None = None,
        net_config: NNUEConfig | None = None,
    ) -> None:
        self.config = config or NNUETrainConfig()
        self.net_config = net_config or NNUEConfig.small()
        self.encoder = NNUEEncoder()
        self.buffer = NNUEReplayBuffer(self.config.buffer_max_size)
        ...
        self.best_net = HiveNNUE(self.net_config).to(self.device)
    ...
    def run(self) -> None:
        ...
        try:
            for iteration in range(
                ...
                print(f"\n{'='*60}")
                print(
                    f"NNUE Iteration {iteration}/{self.config.num_iterations}"
                )
                print(f"{'='*60}")
    ...
    def _self_play_phase(
        self,
    ) -> tuple[list[NNUETrainingExample], SelfPlayStats]:
        """Generate self-play games and collect training examples."""
        all_examples: list[NNUETrainingExample] = []
        stats = SelfPlayStats()
    ...
    def _self_play_game(
        self, net: HiveNNUE, use_endgame: bool = False
    ) -> tuple[list[NNUETrainingExample], GameResult, int]:
        """
        Play one self-play game and return NNUE training examples.

        Each turn:
          1. Encode state as NNUE feature vector
          2. Run MCTS (uses net.predict(features, mask))
          3. Sample/argmax action from policy
          4. Record (features, policy, player)
        ...
        while (
            ...
            features = self.encoder.encode_state(game)
            current_player = game.current_player

            policy = mcts.search(game, move_number=move_number)
            history.append((features, policy, current_player))
        ...
        result = game.result
        examples: list[NNUETrainingExample] = []
        ...
        for features, policy, player in history:
            ...
            examples.append(NNUETrainingExample(features, policy, value))
    ...
    def _train_phase(
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for _ in range(batches_per_epoch):
                features, policies, values = self.buffer.sample_batch(
                ...
                if self.use_amp and self._grad_scaler is not None:
                    with torch.amp.autocast("cuda"):
                        policy_logits, value_pred = new_net(features)
    ...
    def _arena_evaluate(
        self, new_net: HiveNNUE, old_net: HiveNNUE
    ) -> float:
        """
    ...
    def _save_checkpoint(self, net: HiveNNUE, iteration: int) -> str:
        """Save a training checkpoint. Returns the path."""
        path = os.path.join(
            self.config.checkpoint_dir,
            f"hive_nnue_checkpoint_{iteration:04d}.pt",
        )
        checkpoint = {
            "iteration": iteration,
    ...
    def from_checkpoint(
        ...
        Args:
            ...
            config_overrides: Optional dict of NNUETrainConfig overrides.

        Returns:
            An NNUETrainer ready to continue training.
        """
