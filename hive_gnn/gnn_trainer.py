"""
GNN-based AlphaZero trainer for Hive.

Follows the same self-play → train → arena → promote cycle as
hive_engine.trainer.Trainer, but uses GNN graph representations
instead of CNN tensors.

Includes KataGo-inspired improvements:
    - Auxiliary mobility head (per-piece "can this piece move?" prediction)
    - Auxiliary queen surround head (per-piece "adjacent to queen at game end?")
    - Auxiliary mobility end of game head (per-piece "can this piece move at end of game?")
    - Playout cap randomization (varied MCTS simulations per game)

Reuses from hive_engine:
    - compute_loss() / compute_gnn_loss()
    - MCTS (duck-typed with GNNEncoder/HiveGNN)
...
class GNNTrainConfig:
    ...
    # Batched self-play inference
    num_selfplay_workers: int = 16
    ...
    # Checkpointing
    checkpoint_dir: str = "checkpoints_gnn"
...
class GNNTrainer:
    """
    AlphaZero-style self-play trainer using GNN state representation.

    Same pipeline as hive_engine.Trainer:
        1. Self-play → collect (HiveGraph, policy, value, aux) examples
    ...
    def __init__(
        ...
        net_config: GNNNetConfig | None = None,
    ) -> None:
        self.config = config or GNNTrainConfig()
        self.net_config = net_config or GNNNetConfig.small()
        ...
        self.best_net = HiveGNN(self.net_config).to(self.device)
    ...
    def _get_piece_node_list(
        self, game_state: GameState, graph: HiveGraph
    ) -> list[Piece]:
        """
        Get every piece for each piece node, in graph node order.

        Mirrors the iteration order in ``GraphEncoder.encode()``
        (``board.grid.items()``, then bottom-to-top within each stack),
        returning one piece per piece node.
        """
        board = game_state.board
        center_q, center_r = self.encoder._graph_encoder._cached_center(board)
        pieces: list[Piece] = []
        for pos, stack in board.grid.items():
            ...
            for piece in stack:  # bottom to top, all pieces
                pieces.append(piece)
        assert len(pieces) == graph.num_piece_nodes
        return pieces
    ...
    def _compute_mobility_target(
        ...
        """
        Compute per-piece-node binary mobility target.

        For each piece node, 1.0 if that piece has at least one legal
        move (belongs to current player, queen placed, not pinned,
        has destinations, AND is the top piece in its stack), 0.0 otherwise.

        Non-top pieces always get 0.0 since they cannot move.
        """
        board = game_state.board
        color = game_state.current_player
        num_piece = graph.num_piece_nodes
        mobility = np.zeros(num_piece, dtype=np.float32)
                ...
                if (
                    ...
                    if not (
                        ...
                        if len(destinations) > 0:
                            mobility[node_idx] = 1.0
                # else: non-top pieces or wrong color get 0.0 (default)
    ...
    def run(self) -> None:
        ...
        try:
            for iteration in range(
                ...
                print(f"\n{'='*60}")
                print(f"GNN Iteration {iteration}/{self.config.num_iterations}")
                print(f"{'='*60}")
    ...
    def _generate_queen_pressure_examples(
        ...
        """
        examples: list[GNNTrainingExample] = []
        attempts = 0
        ...
        while len(examples) < count and attempts < max_attempts:
            ...
            # Encode state
            graph = self.encoder.encode_state(gs)

            # Auxiliary targets
            mobility = self._compute_mobility_target(gs, graph)
    ...
    def _self_play_phase(
        self,
    ) -> tuple[list[GNNTrainingExample], SelfPlayStats]:
        ...
        server = BatchedInferenceServer(
            net=self.best_net,
            collate_fn=HiveGraphBatch.collate,
            device=self.device,
        ...
        all_examples: list[GNNTrainingExample] = []
        stats = SelfPlayStats()
    ...
    def _self_play_game(
        self,
        net,  # HiveGNN or BatchedPredictor — duck-typed via predict(graph, mask)
        ...
        Each turn:
          1. Encode state as HiveGraph
          2. Compute mobility target
          3. Record piece identity list for queen surround backfill
          4. Run MCTS (uses net.predict(graph, mask))
          5. Sample/argmax action from policy
          6. Record (graph, policy, player, mobility, piece_list)

        After game ends, compute queen surround targets and assign
        value targets.
        """
        ...
        # history: (graph, policy, player, mobility_target, piece_list)
        history: list[
            tuple[HiveGraph, np.ndarray, Color, np.ndarray, list[Piece]]
        ] = []
        ...
        while (
            ...
            graph = self.encoder.encode_state(game)
            current_player = game.current_player

            # Compute auxiliary targets for this position
            mobility = self._compute_mobility_target(game, graph)
            piece_list = self._get_piece_node_list(game, graph)

            policy = mcts.search(game, move_number=move_number)
            history.append(
                (graph, policy, current_player, mobility, piece_list)
            )
        ...
        for graph, policy, player, mobility, piece_list in history:
            # Value target
            ...
            surround = np.zeros(
                (graph.num_piece_nodes, 2), dtype=np.float32
            )
            # Final mobility target for this position's pieces
            final_mob = np.zeros(
                graph.num_piece_nodes, dtype=np.float32
            )
    ...
    def _arena_evaluate(
        self, new_net: HiveGNN, old_net: HiveGNN
    ) -> float:
        """
    ...
    def _save_checkpoint(self, net: HiveGNN, iteration: int) -> str:
        """Save a training checkpoint. Returns the path."""
        path = os.path.join(
            self.config.checkpoint_dir,
            f"hive_gnn_checkpoint_{iteration:04d}.pt",
        )
        checkpoint = {
            "iteration": iteration,
    ...
    def from_checkpoint(
        ...
        Args:
            ...
            config_overrides: Optional dict of GNNTrainConfig overrides.

        Returns:
            A GNNTrainer ready to continue training.
        """
