...
class GNNTrainer:
    ...
    Same pipeline as hive_engine.Trainer:
        1. Self-play → collect (HiveGraph, policy, value) examples
    ...
    def _self_play_game(
        ...
        Each turn:
          ...
          2. Run MCTS (uses net.predict(graph, mask))
          3. Sample/argmax action from policy
          4. Record (graph, policy, player)
        ...
        while (
            ...
            history.append((graph, policy, current_player))
        ...
        for graph, policy, player in history:
            ...
            examples.append(GNNTrainingExample(graph, policy, value))
    ...
    def _train_phase(
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for _ in range(batches_per_epoch):
                graph_batch, policies, values = self.buffer.sample_batch(
                ...
                if self.use_amp and self._grad_scaler is not None:
                    with torch.amp.autocast("cuda"):
                        policy_logits, value_pred = new_net(graph_batch)
