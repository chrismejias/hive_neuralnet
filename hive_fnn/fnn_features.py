...
Produces 140-dim features directly from HiveState + legal moves in a single
CUDA kernel call, bypassing the full encode_states_batch pipeline.

Feature layout (FEAT_DIM = 140):
  ...
  [122:128] sufficient_material -- insufficient/exact/surplus free attackers
                                   per color(2) x bucket(3)
  [128:134] queen_escape     -- no relief/exactly 1/at least 2 fewer occupied
                                queen neighbors per color(2) x bucket(3)
  [134:140] queen_ring_access -- zero/one/multiple distinct free pieces able
                                 to enter an empty opposing queen-ring cell
                                 per color(2) x bucket(3)
"""
